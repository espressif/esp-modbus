...
static bool mbs_try_start_head_transaction(void *ctx, mbs_tcp_port_t *port_obj, uint32_t timeout_ticks)
{
    port_driver_t *drv_obj = MB_GET_DRV_PTR(ctx);
    mb_drv_lock(drv_obj);
    ...
    // Wait until the poll thread releases the semaphore. Do not give it here.
    if (!mb_port_event_res_take(&port_obj->base, timeout_ticks)) {
        return false;
    }
    mb_drv_lock(drv_obj);
    item = transaction_get_first(port_obj->transaction);
...
static void mbs_retrigger_pending_transactions(void *ctx, mbs_tcp_port_t *port_obj)
{
    (void)mbs_try_start_head_transaction(ctx, port_obj, TRANSACTION_TICKS);
}
...
MB_EVENT_HANDLER(mbs_on_recv_data)
{
    ...
    if (pnode) {
        ...
        if (!mbs_try_start_head_transaction(ctx, port_obj, 0)) {
...
MB_EVENT_HANDLER(mbs_on_timeout)
{
    ...
    // Backup start when the completion handoff did not get the semaphore within TRANSACTION_TICKS.
    (void)mbs_try_start_head_transaction(ctx, port_obj, 0);
    mb_drv_lock(drv_obj);
